<script lang="ts">
	...
	let { presets, presetIndex = $bindable(), updatePresetHash, deletePreset }: Props = $props()

	let preset = $derived(presets[presetIndex])
	...
	let usePenPressureForSize = $derived(preset.size_pressure_curve != null)
	let sizeCurveStartPointX = $derived(preset.size_pressure_curve?.[0] ?? 0)
	let sizeCurveStartPointY = $derived(preset.size_pressure_curve?.[1] ?? 0)
	let sizeCurvePointAX = $derived(preset.size_pressure_curve?.[2] ?? 0.25)
	let sizeCurvePointAY = $derived(preset.size_pressure_curve?.[3] ?? 0.25)
	let sizeCurvePointBX = $derived(preset.size_pressure_curve?.[4] ?? 0.75)
	let sizeCurvePointBY = $derived(preset.size_pressure_curve?.[5] ?? 0.75)
	let sizeCurveEndPointX = $derived(preset.size_pressure_curve?.[6] ?? 1)
	let sizeCurveEndPointY = $derived(preset.size_pressure_curve?.[7] ?? 1)

	let softness = $derived(preset.softness)
	let softnessEnabled = $derived(preset.softness != null)
	let usePenPressureForSoftness = $derived(preset.softness_pressure_curve != null)
	let softnessCurveStartPointX = $derived(preset.softness_pressure_curve?.[0] ?? 0)
	let softnessCurveStartPointY = $derived(preset.softness_pressure_curve?.[1] ?? 0)
	let softnessCurvePointAX = $derived(preset.softness_pressure_curve?.[2] ?? 0.25)
	let softnessCurvePointAY = $derived(preset.softness_pressure_curve?.[3] ?? 0.25)
	let softnessCurvePointBX = $derived(preset.softness_pressure_curve?.[4] ?? 0.75)
	let softnessCurvePointBY = $derived(preset.softness_pressure_curve?.[5] ?? 0.75)
	let softnessCurveEndPointX = $derived(preset.softness_pressure_curve?.[6] ?? 1)
	let softnessCurveEndPointY = $derived(preset.softness_pressure_curve?.[7] ?? 1)

	let opacity = $derived(preset.opacity)
	let opacityEnabled = $derived(preset.opacity != null)
	let usePenPressureForOpacity = $derived(preset.opacity_pressure_curve != null)
	let opacityCurveStartPointX = $derived(preset.opacity_pressure_curve?.[0] ?? 0)
	let opacityCurveStartPointY = $derived(preset.opacity_pressure_curve?.[1] ?? 0)
	let opacityCurvePointAX = $derived(preset.opacity_pressure_curve?.[2] ?? 0.25)
	let opacityCurvePointAY = $derived(preset.opacity_pressure_curve?.[3] ?? 0.25)
	let opacityCurvePointBX = $derived(preset.opacity_pressure_curve?.[4] ?? 0.75)
	let opacityCurvePointBY = $derived(preset.opacity_pressure_curve?.[5] ?? 0.75)
	let opacityCurveEndPointX = $derived(preset.opacity_pressure_curve?.[6] ?? 1)
	...
	$effect.pre(() => {
		...
		preset.softness = softnessEnabled ? softness : null
		preset.softness_pressure_curve = usePenPressureForSoftness
			? [
		...
		preset.opacity = opacityEnabled ? opacity : null
		preset.opacity_pressure_curve = usePenPressureForOpacity
			? [
...
{#if usePenPressureForSize && sizeEnabled}
	<CurveEdit
		id="pressure-curve"
		label={tl('action.slider_brush_size') + ' ' + localize('brush_tuna.action.pressure_curve')}
